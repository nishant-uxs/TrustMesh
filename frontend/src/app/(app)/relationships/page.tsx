...
export default function RelationshipsPage() {
  ...
  async function onCreate(e: React.FormEvent) {
    ...
    if (!orgA || !orgB) {
      setError(classifyError(new Error("contract: select two organizations")));
      return;
    }
        ...
        <form onSubmit={onCreate} className="tm-surface space-y-4 rounded-2xl p-5 lg:col-span-2">
          ...
          {orgs.length < 2 ? (
            <p className="text-sm text-slate">
              Register at least two organizations before creating a relationship.
            </p>
          ) : (
            <>
          ...
          {relationships.length === 0 ? (
            <div className="tm-surface rounded-2xl">
              <EmptyState
                ...
                description="Relationships created on-chain will appear in this list."
              />
            </div>
          ) : (
